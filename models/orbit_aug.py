...
hyp = {
    ...
    'aug': {
        'flip': True,
        'translate': 2,
        'orbit_k': 8,  # number of states in the flip+translation orbit
    },
    'net': {
        'widths': {
            'block1': 64,
...
# Each image cycles through these orbit states across epochs; phase is based on image index.
# Augmentation orbit: (flip_flag, dy, dx), where dy/dx are in pixel units,
# and must be within [-translate, translate].
ORBIT_STATES = [
    (0, -2,  0),
    (0,  0,  2),
    (0,  2,  0),
...
ORBIT_K = len(ORBIT_STATES)
assert ORBIT_K == hyp['aug']['orbit_k']
...
class CifarLoader:
    ...
    def __init__(self, path, train=True, batch_size=500, aug=None, drop_last=None, shuffle=None, gpu=0):
        ...
        for k in self.aug.keys():
            assert k in ['flip', 'translate', 'orbit_k'], 'Unrecognized key: %s' % k
    ...
    def __iter__(self):
        ...
        if self.aug.get('translate', 0) > 0:
            ...
            k0 = idxs_all % ORBIT_K
            k = (k0 + self.epoch) % ORBIT_K

            orbit_states = torch.tensor(ORBIT_STATES, device=device)
            flip_flags = orbit_states[k, 0]
            dy_vals = orbit_states[k, 1]
            dx_vals = orbit_states[k, 2]

            images_tmp = torch.empty((N, 3, crop_size, crop_size+2*r), device=device, dtype=padded.dtype)
            images = torch.empty((N, 3, crop_size, crop_size), device=device, dtype=padded.dtype)

            for s in range(-r, r+1):
                mask_y = (dy_vals == s)
                if mask_y.any():
                    y0 = r + s
                    images_tmp[mask_y] = padded[mask_y, :, y0:y0+crop_size, :]

            for s in range(-r, r+1):
                mask_x = (dx_vals == s)
                if mask_x.any():
                    x0 = r + s
                    images[mask_x] = images_tmp[mask_x, :, :, x0:x0+crop_size]

            if self.aug.get('flip', False):
                flip_mask = flip_flags.bool().view(-1, 1, 1, 1)
